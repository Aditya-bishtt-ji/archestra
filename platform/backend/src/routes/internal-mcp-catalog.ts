...
/**
 * Drive reinstall logic for a catalog item whose template just changed.
 * Mirrors the historical inline block from PUT /api/internal_mcp_catalog/:id —
 * extracted so the parent edit can call it once for itself and once per child.
 */
...
/**
 * Pick the columns that should be copied from a parent catalog item to a
 * child (or used as the seed when creating a child). Strips identity,
 * preset-overlay, label/team junctions, and audit columns so they are not
